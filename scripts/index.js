...
const createCard = (vacancy) => {
    ...
    <article class="vacancy">
        <h2 class="vacancy__title"><a href="#">${title}</a></h2>
        ...
        <a class="vacancy__response vacancy__open-modal"
            href="#" data-vacancy="${id}">Откликнуться</a>
...
//фн-я к-я будет брать те данные из глобального переменна data 
const sortData = () => {
...
//Фн-я будет сортировать по времени.
const filterdata = () => {
    const date = new Date();
...
// сортировка по дате и по времени
const optionHandler = () => {
...
const createModal = (data) => {
    ...
    const submitButtonEle = document.createElement('button');
    submitButtonEle.classList.add('modal__response');
    submitButtonEle.textContent = 'Отправить резюме';
...
const searchHandler = () => {
    formSearch.addEventListener('submit', async (e) => {
        ...
        if (textSearch.length > 2) {
            ...
            sortData();
            renderCards(data);
            found.innerHTML = `${declOfNum(data.length, ['вакансия', 'вакансии', 'ваканссий'])} &laquo;${textSearch}&raquo;`;
            formSearch.reset();
        } else {
